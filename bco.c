...
double upgradedSolutionValue(bool *solution, double *fixedCost, int m, int level1, int level2, struct cena **c){
  ...
  int n = level1 * level2;
  bool *used = (bool *)malloc((unsigned long int)level * sizeof(bool));
  assert(used != NULL);
  for (j = 0; j < level; j++){
    used[j] = false;
  }
...
int *invertbco(bool *solution, int level1, int level2){
...
//algoritam optimizacije kolonijom pcela

struct resenje bco(double *fixedCost, int m, int level1, int level2, struct cena **c){
  ...
  do{
    ...
    for (u = 0; u < NC; u++){
      //forward pass
      for (b = 0; b < B; b++){
        a = invertbco(solution[b], level1, level2);
        if (a[0] == -1 || a[1] == -1){
          b--;
        }
      }
...
//main funkcija gde se vrsi ucitavanje podataka iz fajla i poziv algoritma BCO kao i ispis resenja
...
int main(int argc, char **argv){
  ...
  double *d = (double *)malloc((unsigned long int)nbCustomers * sizeof(double));
  assert(d != NULL);
  ...
  double *fixedCost = (double *)malloc((unsigned long int)level * sizeof(double));
  ...
  double **a = (double **)malloc((unsigned long int)nbCustomers * sizeof(double *));
  assert(a != NULL);
  for (int i = 0; i < nbCustomers; i++){
    a[i] = (double *)malloc((unsigned long int)level1 * sizeof(double));
    assert(a[i] != NULL);
  ...
  double **b = (double **)malloc((unsigned long int)level1 * sizeof(double *));
  assert(b != NULL);
  for (int j1 = 0; j1 < level1; j1++){
    b[j1] = (double *)malloc((unsigned long int)level2 * sizeof(double));
    assert(b[j1] != NULL);
  ...
  struct cena **c = (struct cena **)malloc((unsigned long int)nbCustomers * sizeof(struct cena *));
  assert(c != NULL);
  for (int i = 0; i < nbCustomers; i++){
    c[i] = (struct cena *)malloc((unsigned long int)n * sizeof(struct cena));
    assert(c[i] != NULL);
    for (int j1 = 0; j1 < level1; j1++){
      for (int j2 = 0; j2 < level2; j2++){
  ...
  struct resenje solutionValue = bco(fixedCost, nbCustomers, level1, level2, c);
